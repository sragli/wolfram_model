defmodule WolframModel.MatcherTest do
  ...
  test "single-hyperedge mapping is deterministic" do
    ...
    # The replacement should map :a -> :v1 (sorted mapping) and keep a single vertex
    assert Enum.any?(Hypergraph.hyperedges(next.hypergraph), fn he -> MapSet.size(he) == 1 end)
  end
  ...
  test "two-hyperedge pattern mapping maps shared vertex correctly" do
    ...
    rule = %{
      pattern: [MapSet.new([:a, :b]), MapSet.new([:b, :c])],
      replacement: [MapSet.new([:a, :c])],
      name: "join"
    }
